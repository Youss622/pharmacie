...
export default function Stocks() {
  ...
  const [formData, setFormData] = useState({ product_id: '', name: '', stock: 0, lot: '', expiry_date: '', price: 0, currency: 'GNF', category: 'Antalgique' });

  useEffect(() => { loadProducts(); }, []);

  const loadProducts = () => { api.get('products/').then(res => setProducts(res.data)).catch(console.error); };
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Êtes-vous sûr de vouloir supprimer ce médicament ?")) {
      try { await api.delete(`products/${id}/`); loadProducts(); } catch (err) { alert("Erreur lors de la suppression."); }
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editingId) { await api.put(`products/${editingId}/`, formData); } else { await api.post('products/', formData); }
      setShowForm(false);
      setEditingId(null);
      setFormData({ product_id: '', name: '', stock: 0, lot: '', expiry_date: '', price: 0, currency: 'GNF', category: 'Antalgique' });
      loadProducts();
    } catch (err) {
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <button onClick={() => showForm ? handleCancel() : setShowForm(true)}>{showForm ? 'Annuler' : '+ Nouveau Produit'}</button>
      </div>
      ...
      <div className="glass-panel">
        {products.length === 0 ? <p>Le stock est vide.</p> : (
          <table>
            <thead>
              <tr>
                <th>Réf</th>
            ...
            <tbody>
              {products.map(p => (
                <tr key={p.id}>
                  ...
                  <td>{p.name}</td>
                  <td><span style={{background:'#e2e8f0', padding:'2px 8px', borderRadius:'12px', fontSize:'0.8rem', color:'#475569', fontWeight:600}}>{p.category || 'Générique'}</span></td>
                  <td><span style={{ color: p.stock <= 10 ? 'var(--danger)' : 'inherit', fontWeight: p.stock <= 10 ? 600 : 400 }}>{p.stock} {p.stock <= 10 && '⚠️'}</span></td>
                  <td>{p.price} {p.currency}</td>
                  <td>{p.lot}</td>
                  <td>{new Date(p.expiry_date).toLocaleDateString()}</td>
                  <td style={{ whiteSpace: 'nowrap' }}>
                    <button style={{ background: '#10B981', padding: '0.4rem 0.6rem', marginRight: '0.5rem', fontSize: '0.8rem', borderRadius: '4px' }} onClick={() => handleEdit(p)}>Modifier</button>
                    <button style={{ background: '#EF4444', padding: '0.4rem 0.6rem', fontSize: '0.8rem', borderRadius: '4px' }} onClick={() => handleDelete(p.id)}>Supprimer</button>
                  </td>
                </tr>
              ))}
            </tbody>
